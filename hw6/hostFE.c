...
{
    cl_int status;
    const size_t img_size = sizeof(float)*imageHeight*imageWidth;
    const size_t filter_size = sizeof(float)*filterWidth*filterWidth;
    ...
    clEnqueueWriteBuffer(cq,c_input,CL_TRUE,0,img_size,(void *)inputImage,0,NULL,NULL);
    clEnqueueWriteBuffer(cq,c_filter,CL_TRUE,0,filter_size,(void *)filter,0,NULL,NULL);
    ...
    const size_t globalws[2] = {imageWidth,imageHeight};
    clEnqueueNDRangeKernel(cq,c_kernel,2,0,globalws,NULL,0,NULL,NULL);
    clEnqueueReadBuffer(cq,c_output,CL_TRUE,0,img_size,(void *)outputImage,0,NULL,NULL);
}
